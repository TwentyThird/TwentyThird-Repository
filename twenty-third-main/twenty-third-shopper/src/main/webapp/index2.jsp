...
<body>
<h3>boot+dobbu+zookper</h3>
<table id="usertable2"></table><br><br>
...
$(function (){
		$("#usertable2").bootstrapTable({
		 url:"<%=request.getContextPath()%>/user/selectuserBoot.action",	
		 dataType:"json",
		 type:"get",
		...
		columns:[[
			        ...
			        {field:'id',title:'操作',width:100,
			        	formatter:function(value,row,index){   //  格式化  当前单元格内容
							return "<a id='upd' onclick='updBoot("+value+")'>修改</a>";
						}
			        },    
		    ]],
		         //传递参数（*）
...
function updBoot(id){
	...
	$.ajax({
		url:"<%=request.getContextPath()%>/user/selectByIdBoot.action?id="+id,
		type:"post",
		dataType:"json",
...
function update(){
	$.ajax({
		url:"<%=request.getContextPath()%>/user/updInfoboot.action",
		type:"post",
		data:$("#updform2").serialize(),
		dataType:"json",
		success:function(data){
			
			alert("修改成功")
			$("#updDiv").hide();
			$("#usertable2").bootstrapTable('refresh');
		},
		error:function(){
			alert("ajax");
		}
